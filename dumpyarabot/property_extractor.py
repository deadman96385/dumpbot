...
class PropertyExtractor:
    ...
    async def extract_properties(self) -> Dict[str, Any]:
        ...
        props["brand"] = await self._extract_brand()
    ...
    async def _extract_brand(self) -> Optional[str]:
        ...
        for patterns, paths in patterns_and_paths:
            result = await self._search_property(patterns, paths)
            if result:
                # Special handling for OPPO brand
                if result == "OPPO":
                    # Try to get more specific brand from system
                    specific_brand = await self._search_property(
                        ["ro.product.system.brand"],
                        ["vendor/euclid/*/build.prop"]
                    )
                    if specific_brand:
                        result = specific_brand
                return result
        ...
        manufacturer = await self._extract_manufacturer()
        return manufacturer
        ...
        for key in ["repo_subgroup", "repo_name", "manufacturer"]:
            if props.get(key):
                props[key] = (
                    props[key].lower()
                    .replace("_", "-")
                    .encode("ascii", "ignore").decode()[:35]
                )

        # Format platform
        if props.get("platform"):
            props["platform"] = (
                props["platform"].lower()
                .replace("_", "-")
                .encode("ascii", "ignore").decode()[:35]
            )
