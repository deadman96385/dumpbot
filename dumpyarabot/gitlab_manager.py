...
class GitLabManager:
    ...
    def __init__(self, work_dir: str):
        ...
        self.gitlab_server = "dumps.tadiphone.dev"  # From extract_and_push.sh
        self.push_host = "dumps"
        self.org = "dumps"
        self.parent_group_id = 64  # From extract_and_push.sh
    ...
    async def create_and_push_repository(
        ...
        project_id = await self._ensure_project_exists(group_id, repo_name, dumper_token)
    ...
    async def _ensure_project_exists(self, group_id: int, repo_name: str, dumper_token: str) -> int:
        ...
        async with httpx.AsyncClient(verify=False) as client:
            ...
            response = await client.get(
                f"https://{self.gitlab_server}/api/v4/projects/{self.org}%2f{group_id}%2f{repo_name}",
                headers={"Authorization": f"Bearer {dumper_token}"},
                timeout=30.0
            )
