...
class GitLabManager:
    ...
    async def create_and_push_repository(
        self, device_props: Dict[str, Any], dumper_token: str
    ) -> Tuple[str, str]:
        ...
        console.print(
            f"[blue]Creating GitLab repository: {self.org}/{repo_subgroup}/{repo_name}[/blue]"
        )
        ...
        project_id = await self._ensure_project_exists(
            group_id, repo_name, dumper_token, repo_subgroup
        )
        ...
        repo_url = await self._push_to_gitlab(
            project_id, repo_subgroup, repo_name, branch, dumper_token
        )
    ...
    async def _ensure_subgroup_exists(
        self, subgroup_name: str, dumper_token: str
    ) -> int:
        ...
        async with httpx.AsyncClient(verify=False) as client:
            ...
            if response.status_code == 200:
                ...
                console.print(
                    f"[green]Subgroup {subgroup_name} exists with ID: {group_id}[/green]"
                )
                return group_id
            ...
            if create_response.status_code in [200, 201]:
                ...
                group_id = group_data["id"]
                console.print(
                    f"[green]Created subgroup {subgroup_name} with ID: {group_id}[/green]"
                )
                return group_id
            else:
                raise Exception(
                    f"Failed to create subgroup {subgroup_name}: {create_response.text}"
                )

    async def _ensure_project_exists(
        self, group_id: int, repo_name: str, dumper_token: str, repo_subgroup: str
    ) -> int:
        ...
        async with httpx.AsyncClient(verify=False) as client:
            ...
            if response.status_code == 200:
                ...
                console.print(
                    f"[green]Project {repo_name} exists with ID: {project_id}[/green]"
                )
                return project_id
            ...
            if create_response.status_code in [200, 201]:
                ...
                project_id = project_data["id"]
                console.print(
                    f"[green]Created project {repo_name} with ID: {project_id}[/green]"
                )
                return project_id
            else:
                raise Exception(
                    f"Failed to create project {repo_name}: {create_response.text}"
                )

    async def _branch_exists(
        self, project_id: int, branch: str, dumper_token: str
    ) -> bool:
    ...
    async def _set_default_branch(
        ...
        async with httpx.AsyncClient(verify=False) as client:
            ...
            if response.status_code == 200:
                ...
                console.print(
                    f"[yellow]Failed to set default branch: {response.text}[/yellow]"
                )
    ...
    async def send_channel_notification(
        ...
        if is_whitelisted and not add_blacklist:
            console.print(
                "[blue]Skipping channel notification (whitelisted and not blacklisted)[/blue]"
            )
            return
        ...
        async with httpx.AsyncClient(verify=False) as client:
            ...
            if response.status_code == 200:
                ...
                console.print(
                    f"[yellow]Failed to send channel notification: {response.text}[/yellow]"
                )
    ...
    async def check_whitelist(self, url: str) -> bool:
        ...
        try:
            ...
            for domain in whitelist_domains:
                if domain in url:
                    console.print(
                        f"[green]URL is whitelisted (domain: {domain})[/green]"
                    )
                    return True
