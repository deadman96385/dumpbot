...
class WorkerSettings:
    ...
    functions = [process_firmware_dump]
...
class ARQPool:
    ...
    async def get_pool(self) -> ArqRedis:
        ...
        if self._pool is None or self._closed:
            self._pool = await arq.connections.create_pool(
    ...
    async def enqueue_job(
        self, function_name: str, *args, job_id: Optional[str] = None, **kwargs: Any
    ) -> str:
        """Enqueue a job using ARQ."""
        pool = await self.get_pool()

        result_job_id = await pool.enqueue_job(
            function_name,
            *args,
            _job_id=job_id,
        ...
        # ARQ returns a job ID string
        final_job_id = str(result_job_id) if result_job_id else (job_id or "unknown")

        console.print(
            f"[green]Enqueued ARQ job {final_job_id} ({function_name}) to queue {WorkerSettings.queue_name}[/green]"
        )
        return final_job_id
    ...
    async def get_job_status(self, job_id: str) -> Optional[dict]:
        ...
        try:
            ...
            job_result = await JobResult.create(pool, job_id)
            ...
            return {
                ...
                "enqueue_time": job_result.enqueue_time.isoformat()
                if job_result.enqueue_time
                else None,
                "start_time": job_result.start_time.isoformat()
                if job_result.start_time
                else None,
                "finish_time": job_result.finish_time.isoformat()
                if job_result.finish_time
                else None,
                "success": job_result.success,
            }
        except Exception as e:
            console.print(
                f"[yellow]Could not get job status for {job_id}: {e}[/yellow]"
            )
            return None
    ...
    async def cancel_job(self, job_id: str) -> bool:
        ...
        try:
            # Try to abort the job (method may not exist in all ARQ versions)
            abort_method = getattr(pool, 'abort_job', None)
            if abort_method:
                success = await abort_method(job_id)
            else:
                console.print(f"[yellow]abort_job not available in this ARQ version[/yellow]")
                success = False
