...
def load_test_config():
    ...
    test_overrides = {
        ...
        "allowed_chats": [int(os.getenv("TEST_DUMP_CHAT_ID", 0))]
        if os.getenv("TEST_DUMP_CHAT_ID")
        else None,
        "request_chat_id": int(os.getenv("TEST_REQUEST_CHAT_ID", 0))
        if os.getenv("TEST_REQUEST_CHAT_ID")
        else None,
        "review_chat_id": int(os.getenv("TEST_REVIEW_CHAT_ID", 0))
        if os.getenv("TEST_REVIEW_CHAT_ID")
        else None,
...
def mock_telegram_update(faker):
    ...
    update.effective_message.from_user.id = faker.random_int(
    ...
    update.effective_message.reply_to_message.text = (
        "Sample replied message with Request ID: abc12345"
    )
...
def mock_message_queue():
    ...
    queue.publish_and_return_placeholder = AsyncMock(
