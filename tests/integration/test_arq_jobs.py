...
ro.build.id=TEST123""",
    ...
    async def test_process_firmware_dump_basic(self, mock_firmware_path):
        ...
        with patch(
            "dumpyarabot.arq_jobs.FirmwareDownloader"
        ) as mock_downloader_class, patch(
            "dumpyarabot.arq_jobs.FirmwareExtractor"
        ) as mock_extractor_class, patch(
            "dumpyarabot.arq_jobs.PropertyExtractor"
        ) as mock_property_class, patch(
            "dumpyarabot.arq_jobs.GitLabManager"
        ) as mock_gitlab_class, patch(
            "dumpyarabot.arq_jobs.settings"
        ) as mock_settings, patch(
            "dumpyarabot.message_queue.message_queue.send_status_update"
        ) as mock_status_update, patch(
            "dumpyarabot.message_queue.message_queue.send_cross_chat_edit"
        ) as mock_cross_chat_edit:
            # Setup mocks
            mock_downloader = MagicMock()
            mock_downloader.download_firmware = AsyncMock(
                return_value=(mock_firmware_path, "test_firmware.zip")
            )
            ...
            mock_property_extractor.extract_properties = AsyncMock(
                return_value={
                    "brand": "TestBrand",
                    "model": "TestModel",
                    "android_version": "13",
            ...
            mock_gitlab.create_and_push_repository = AsyncMock(
                return_value=("https://test.gitlab.com/test/repo", "/tmp/extracted")
            )
            ...
            total_updates = (
                mock_status_update.call_count + mock_cross_chat_edit.call_count
            )
