...
class TestRequestMessageHandler:
    ...
    @pytest.mark.asyncio
    async def test_handle_request_message_valid(
        self, mock_telegram_update, mock_telegram_bot
    ):
        ...
        mock_telegram_update.effective_message.text = (
            "#request https://example.com/firmware.zip please dump this"
        )
        mock_telegram_update.effective_user.username = "testuser"
        mock_telegram_update.effective_user.id = 12345

        with patch("dumpyarabot.moderated_handlers.settings") as mock_settings, patch(
            "dumpyarabot.moderated_handlers.message_queue"
        ) as mock_queue, patch(
            "dumpyarabot.moderated_handlers.url_utils.validate_and_normalize_url"
        ) as mock_validate, patch(
            "dumpyarabot.moderated_handlers.utils.generate_request_id"
        ) as mock_gen_id, patch(
            "dumpyarabot.moderated_handlers.ReviewStorage"
        ) as mock_storage:
            # Setup mocks
            ...
            mock_queue.publish_and_return_placeholder = AsyncMock(
                return_value=MagicMock(message_id=456)
            )
            ...
            submission_call = mock_queue.publish_and_return_placeholder.call_args_list[
                1
            ]
    ...
    @pytest.mark.asyncio
    async def test_handle_request_message_wrong_chat(
        self, mock_telegram_update, mock_telegram_bot
    ):
        ...
        mock_telegram_update.effective_message.text = (
            "#request https://example.com/firmware.zip"
        )
    ...
    @pytest.mark.asyncio
    async def test_handle_request_message_invalid_url(
        self, mock_telegram_update, mock_telegram_bot
    ):
        ...
        with patch("dumpyarabot.moderated_handlers.settings") as mock_settings, patch(
            ...
            assert (
                "error occurred" in call_args[1]["text"].lower()
                or "invalid" in call_args[1]["text"].lower()
            )

    @pytest.mark.asyncio
    async def test_handle_request_message_no_pattern(
        self, mock_telegram_update, mock_telegram_bot
    ):
        ...
        mock_telegram_update.effective_message.text = (
            "Just a regular message https://example.com/firmware.zip"
        )
...
class TestCallbackQueryHandler:
    ...
    @pytest.mark.asyncio
    async def test_handle_callback_query_accept(
        self, mock_telegram_update, mock_telegram_bot
    ):
        ...
        with patch("dumpyarabot.moderated_handlers.settings") as mock_settings, patch(
            ...
            mock_storage.get_options_state.return_value = MagicMock(
                alt=False, force=False, privdump=False
            )
    ...
    @pytest.mark.asyncio
    async def test_handle_callback_query_reject(
        self, mock_telegram_update, mock_telegram_bot
    ):
...
class TestAcceptCommand:
    ...
    async def test_accept_command_valid(self, mock_telegram_update, mock_telegram_bot):
        ...
        mock_telegram_update.effective_message.reply_to_message.text = (
            "Some message with Request ID: abc12345"
        )
        ...
        with patch("dumpyarabot.moderated_handlers.settings") as mock_settings, patch(
            ...
            "dumpyarabot.moderated_handlers.secrets.token_hex"
        ) as mock_token, patch(
            "dumpyarabot.moderated_handlers._cleanup_request"
        ) as mock_cleanup:
            mock_settings.REVIEW_CHAT_ID = mock_telegram_update.effective_chat.id
            mock_storage.get_pending_review.return_value = MagicMock(
                url="https://example.com/firmware.zip",
    ...
    @pytest.mark.asyncio
    async def test_accept_command_invalid_request(
        self, mock_telegram_update, mock_telegram_bot
    ):
        ...
        with patch("dumpyarabot.moderated_handlers.settings") as mock_settings, patch(
            ...
            call_args = mock_queue.send_error.call_args
            assert (
                "not found" in call_args[1]["text"].lower()
                or "expired" in call_args[1]["text"].lower()
            )

    @pytest.mark.asyncio
    async def test_accept_command_no_args(
        self, mock_telegram_update, mock_telegram_bot
    ):
...
class TestRejectCommand:
    ...
    @pytest.mark.asyncio
    async def test_reject_command_no_reason(
        self, mock_telegram_update, mock_telegram_bot
    ):
...
class TestUtilityFunctions:
    ...
    async def test_cleanup_request(self, mock_telegram_update):
        ...
        with patch("dumpyarabot.moderated_handlers.ReviewStorage") as mock_storage:
            ...
            mock_storage.remove_pending_review.assert_called_once_with(
                context, "test123"
            )
            mock_storage.remove_options_state.assert_called_once_with(
                context, "test123"
            )
