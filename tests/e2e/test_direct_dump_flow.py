...
class TestModeratedRequestFlowE2E:
    ...
    async def test_complete_moderated_request_flow_real_e2e(
        ...
        request_msg = await bot_application.bot.send_message(
            chat_id=request_chat_id, text=f"#request {firmware_url}"
        )
        ...
        review_msg = await message_monitor.wait_for_message(
            review_chat_id,
            lambda msg: (
                "Accept" in (msg.text or "")
                and "Reject" in (msg.text or "")
                and firmware_url in (msg.text or "")
            ),
            timeout=10,
        )
        ...
        request_id_match = re.search(
            r"Request ID: ([a-f0-9]{8})", review_msg.text or ""
        )
        ...
        options_msg = await message_monitor.wait_for_message(
            review_chat_id,
            lambda msg: (
                "Configure options" in (msg.text or "")
                and msg.message_id == review_msg.message_id
            ),  # Same message edited
            timeout=10,
        )
        ...
        assert any(
            "Alternative Dumper" in text for text in button_texts
        ), "Alternative Dumper option not found"
        ...
        job_id_match = re.search(
            r"job queued with ID ([a-f0-9]+)", job_queued_msg.text or ""
        )
        ...
        review_message_found = any(
            msg.message_id == review_msg.message_id for msg in recent_messages[-5:]
        )
    ...
    async def test_moderated_request_invalid_url(
        ...
        request_msg = await bot_application.bot.send_message(
            chat_id=request_chat_id, text=f"#request {invalid_url}"
        )
    ...
    async def test_moderated_request_with_alt_dumper_option(
        ...
        updated_options_msg = await message_monitor.wait_for_message(
            review_chat_id,
            lambda msg: (
                "✅ Alternative Dumper" in (msg.text or "")
                and msg.message_id == options_msg.message_id
            ),
            timeout=10,
        )
    ...
    async def test_moderated_request_with_private_dump_option(
        ...
        acceptance_msg = await message_monitor.wait_for_message(
            request_chat_id,
            lambda msg: "under further review for private processing"
            in (msg.text or ""),
            timeout=10,
        )
    ...
    async def test_moderated_request_with_multiple_options(
        ...
        final_options_msg = await message_monitor.wait_for_message(
            ...
            lambda msg: (
                "✅ Alternative Dumper" in (msg.text or "")
                and "⚡ Force Re-Dump" in (msg.text or "")
                and "🔒 Private Dump" in (msg.text or "")
                and msg.message_id == options_msg.message_id
            ),
            timeout=10,
        )
    ...
    async def test_job_processing_status_updates(
        ...
        print(
            f"Job processing status test completed - Job {job_id} queued successfully"
        )
        await message_monitor.stop_monitoring()
    ...
    async def test_job_progress_percentage_updates(
        ...
        review_msg = await message_monitor.wait_for_message(
            review_chat_id, lambda msg: "Accept" in (msg.text or ""), timeout=10
        )
        request_id_match = re.search(
            r"Request ID: ([a-f0-9]{8})", review_msg.text or ""
        )
    ...
    async def test_job_device_info_extraction_display(
        ...
        assert (
            job_queued_msg
        ), "Job queuing failed - device info display depends on this"
    ...
    async def test_job_cancellation_workflow(
        ...
        cancel_msg = await bot_application.bot.send_message(
            chat_id=review_chat_id, text=f"/cancel {job_id}"
        )
    ...
    async def test_firmware_download_failure_handling(
        ...
        invalid_url = (
            "https://definitely-not-a-valid-firmware-url-12345.com/firmware.zip"
        )
    ...
    @pytest.mark.asyncio
    async def test_unauthorized_chat_access(self, bot_application):
    ...
    async def test_non_admin_user_restrictions(
        ...
        assert (
            options_msg
        ), "Admin should still be able to accept after non-admin attempt"
    ...
    async def test_malformed_callback_data_handling(
        ...
        fake_message = await bot_application.bot.send_message(
            chat_id=review_chat_id, text="Test message for callback injection"
        )
    ...
    async def test_request_message_validation(
        ...
        for invalid_request in invalid_requests:
            request_msg = await bot_application.bot.send_message(
                chat_id=request_chat_id, text=invalid_request
            )
    ...
    async def test_multiple_concurrent_requests(
        ...
        for url in firmware_urls:
            msg = await bot_application.bot.send_message(
                chat_id=request_chat_id, text=f"#request {url}"
            )
        ...
        for i in range(len(firmware_urls)):
            review_msg = await message_monitor.wait_for_message(
                ...
                lambda msg: (
                    "Accept" in (msg.text or "")
                    and "Reject" in (msg.text or "")
                    and len(review_messages) < len(firmware_urls)
                ),
                timeout=15,
            )
            if review_msg:
        ...
        assert (
            len(review_messages) > 0
        ), "No review messages found for concurrent requests"
        ...
        print(
            f"Multiple concurrent requests test completed - {len(review_messages)} requests processed"
        )
        await message_monitor.stop_monitoring()
    ...
    async def test_cross_chat_message_threading(
        ...
        assert (
            acceptance_msg.reply_to_message
        ), "Acceptance message should reply to original request"
        assert (
            acceptance_msg.reply_to_message.message_id == request_msg.message_id
        ), "Wrong message threaded"
    ...
    async def test_admin_workflow_efficiency(
        ...
        request_id_match = re.search(
            r"Request ID: ([a-f0-9]{8})", review_msgs[0].text or ""
        )
        if request_id_match:
            request_id = request_id_match.group(1)
        ...
        if len(review_msgs) > 1:
            request_id_match = re.search(
                r"Request ID: ([a-f0-9]{8})", review_msgs[1].text or ""
            )
            if request_id_match:
                request_id = request_id_match.group(1)
...
async def cleanup_test_chat(bot, chat_id, keep_recent=1):
    ...
    try:
        ...
        chat_messages = [
            u for u in updates if u.message and str(u.message.chat.id) == str(chat_id)
        ]
